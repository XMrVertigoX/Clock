...
HT16K33_Display::HT16K33_Display(uint8_t address) {
    ...
    for (uint8_t i = 0; i < sizeof(initSequence); i++) {
        ...
        I2cMaster::getInstance()->writeBytes(address, &initSequence[i], 1);
    }
...
void HT16K33_Display::setBrightness(uint8_t brightness) {
    ...
    I2cMaster::getInstance()->startTransmission();
    I2cMaster::getInstance()->writeBytes(address, &brightness, 1);
    I2cMaster::getInstance()->stopTransmission();
}
...
void HT16K33_Display::toggleColon() {
    ...
    I2cMaster::getInstance()->startTransmission();
    I2cMaster::getInstance()->writeBytes(address, &tmp[0], sizeof(tmp[0]));
    I2cMaster::getInstance()->readBytes(address, &tmp[1], sizeof(tmp[1]));
    tmp[1] = ~tmp[1];
    I2cMaster::getInstance()->startTransmission();
    I2cMaster::getInstance()->writeBytes(address, tmp, sizeof(tmp));
    I2cMaster::getInstance()->stopTransmission();
}
