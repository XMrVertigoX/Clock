...
HT16K33_Display::HT16K33_Display(I2cMaster* i2cMaster, uint8_t address) {
...
void HT16K33_Display::toggleColon() {
    ...
    i2cMaster->startTransmission();
    i2cMaster->writeBytes(address, &tmp[0], sizeof(tmp[0]));
    i2cMaster->readBytes(address, &tmp[1], sizeof(tmp[1]));
    tmp[1] = ~tmp[1];
    i2cMaster->startTransmission();
